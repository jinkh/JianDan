...
- (void)dismiss {
    ...
    //使用默认的view
    if (self.alertHeight) {
        [self animateDefaultWithY:SCREEN_HEIGHT+30 removeFromSuperView:YES];
        return;
    }
    //使用自定义NIB
    [self animateWithY:SCREEN_HEIGHT+30 removeFromSuperView:YES];
}
...
- (void)animateWithY:(NSInteger)y removeFromSuperView:(BOOL)isRemove {
    ...
    [self layoutIfNeeded];
    [UIView animateWithDuration:.5 delay:0.0 usingSpringWithDamping:0.6 initialSpringVelocity:0 options:0 animations:^{
...
- (id)initWithTitle:(NSString *)title
   ...
   rightButtonTitle:(NSString *)rigthTitle {
    if (self = [self init]) {
        ...
        alertTitleLabel.dk_textColorPicker = Text_Title;
        ...
        if (content) {
            ...
            alertContentLabel.dk_textColorPicker = Text_Title;
            alertContentLabel.font = [UIFont systemFontOfSize:17.0f];
        ...
        if (leftTitle) {
            leftBtn.frame = CGRectMake(4, CGRectGetMaxY(content.length ? alertContentLabel.frame : alertTitleLabel.frame) + 20, (contentW - 18) / 2, 40 );
            [leftBtn setTitle:leftTitle forState:UIControlStateNormal];
            leftBtn.titleLabel.font = [UIFont systemFontOfSize:18.0f];
            [leftBtn dk_setTitleColorPicker:Text_Title forState:UIControlStateNormal];
            leftBtn.dk_backgroundColorPicker = Dialog_Btn;
        ...
        rightBtn.frame = CGRectMake(leftTitle.length ? CGRectGetMaxX(leftBtn.frame) + 10 : 4, CGRectGetMaxY(alertContentLabel ? alertContentLabel.frame : alertTitleLabel.frame) + 20, leftTitle.length ? (contentW - 18) / 2 : (contentW - 8), 40);
        [rightBtn setTitle:rigthTitle forState:UIControlStateNormal];
        rightBtn.titleLabel.font = [UIFont systemFontOfSize:18.0f];
        [rightBtn dk_setTitleColorPicker:Text_Title forState:UIControlStateNormal];
        rightBtn.dk_backgroundColorPicker = Dialog_Btn;
        [rightBtn addTarget:self action:@selector(rightBtnClicked:) forControlEvents:UIControlEventTouchUpInside];
        [contentView addSubview:rightBtn];

        contentView.frame = CGRectMake(10, 15, contentW, CGRectGetMaxY(rightBtn.frame));
        _alertHeight = CGRectGetMaxY(contentView.frame) + 15;
...
- (void)showDefaultAlert {
    ...
    [[self topView] addSubview:self];
    [UIView animateWithDuration:.5 delay:0.0 usingSpringWithDamping:0.6 initialSpringVelocity:0 options:0
                     animations:^{
