...
{
    return [_gridController  getImageAtIndex:index];
}
...
- (void)setPhotoSelected:(BOOL)selected atIndex:(NSUInteger)index {
    ...
    NSLog(@"_selectCount = %d", _selectCount);
    if (_displaySelectionButtons) {
...
- (void)updateNavigation {
    ...
    if (_gridController) {
        if (_gridController.selectionMode) {
            ...
            NSString *photosText;
            if (numberOfPhotos == 1) {
                photosText = NSLocalizedString(@"照片", @"Used in the context: '1 photo'");
            } else {
                photosText = NSLocalizedString(@"照片", @"Used in the context: '3 photos'");
            }
...
- (void)selectedButtonTapped:(id)sender {
    ...
    if (selectedButton.selected == NO && _selectCount >= _maxSelectCount) {
        return;
    }
